...
def test_site_activate_writes_app_config(tmp_path: Path, monkeypatch) -> None:
    ...
    config.write_site_config(_site_config(tmp_path), path=paths.SITES_DIR / "example.toml")

    result = CliRunner().invoke(app, ["site", "activate", "example"])
    ...
    assert config.read_site_config("example", path=paths.SITES_DIR / "example.toml").mode == "app"
...
def test_site_deactivate_writes_landing_config(tmp_path: Path, monkeypatch) -> None:
    ...
    config.write_site_config(
        ...
        path=paths.SITES_DIR / "example.toml",
    )

    result = CliRunner().invoke(app, ["site", "deactivate", "example"])
    assert result.exit_code == 0

    rendered = (Path(server_config.nginx_sites_available) / "example.com.conf").read_text(encoding="utf-8")
    assert "root " + str(tmp_path / "site" / "_cherve" / "landing") in rendered
    assert config.read_site_config("example", path=paths.SITES_DIR / "example.toml").mode == "landing"
...
def test_site_tls_enable_updates_config(tmp_path: Path, monkeypatch) -> None:
    ...
    paths.SITES_DIR.mkdir(parents=True, exist_ok=True)
    config.write_site_config(
        _site_config(tmp_path),
        path=paths.SITES_DIR / "example.toml",
    )

    input_data = "\n".join(["y", "admin@example.com"])
    result = CliRunner().invoke(app, ["site", "tls", "enable", "example", "example.com"], input=input_data)
    assert result.exit_code == 0

    assert any(cmd[0] == "certbot" for cmd in run_calls)
    loaded = config.read_site_config("example", path=paths.SITES_DIR / "example.toml")
    assert loaded.domains[0].tls_enabled is True
    assert loaded.domains[0].ssl_certificate.endswith("/etc/letsencrypt/live/example.com/fullchain.pem")
